...
type LocalServiceName = 'Ollama' | 'LMStudio' | 'PocketBase';
...
const SERVICE_CONFIGS: Record<LocalServiceName, { url: string; icon: IconType; healthPath: string }> = {
  Ollama: {
...
const ServiceStatusTab = () => {
  ...
  const fetchAllStatuses = useCallback(async () => {
    ...
    const statuses = await Promise.all(
      (Object.keys(SERVICE_CONFIGS) as LocalServiceName[]).map((name) => checkService(name)),
    );
      ...
      >
        ...
        <div className="p-4 bg-bolt-elements-background-depth-2 rounded-lg">
          ...
          <div className="space-y-2 text-xs text-bolt-elements-textSecondary">
            ...
            <p>
              <strong>PocketBase:</strong> Auto-started by the dev server on port 8090
            </p>
          </div>
        </div>
      </motion.div>
