...
import type { ActionAlert, BoltAction, DeployAlert, FileHistory, PocketBaseAlert } from '~/types/actions';
...
import { runPocketBaseActionHandler } from './handlers/pocketbase-handler';
  ...
  async #executeAction(actionId: string, isStreaming: boolean = false) {
    ...
    try {
      switch (action.type) {
        ...
        case 'pocketbase': {
          try {
            await runPocketBaseActionHandler({
              action,
              logger,
              onPocketBaseAlert: this.onPocketBaseAlert,
